...
""" Sample update query 
db.slack.update( {name:"Netherlands Antilles Grill",city:"El Cerrito"},
{$push:{"reviews":{rating:4,comments:"Good"}}})
"""
def gen_post(match, post):
    '''
    Generate "insert a new post" query
    '''
    query_file.write('db.slack.update(' + match +
                     ', {$push:{"posts":' + post +
                     '}});\n')
...
def generate_queries(to_generate):
    ...
    while query_ct < to_generate:
        if random() < random():
            ...
            gen_post(gen_id_doc(),str(gen_single_post()))
        query_ct += 1
